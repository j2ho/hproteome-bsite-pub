...
def seq_registerligands(uniid, domainname, seqcsv):
    seqligdic = readcsv(seqcsv)
    sites = {}
    for lig in seqligdic:
        singledic = seqligdic[lig]
    ...
    for siterank in sites:
        s = Site(rank=siterank, method='sequence', coordinate=sites[siterank],uniquename='%s_seq_site%s'%(domainname,siterank))
        s.save()
    ...
    for lig in seqligdic:
        ...
        siterank = singledic['siterank']
        ligcoord = singledic['ligcoord']
        filepath = 'sitedb/%s/%s_seq_%s.mol2'%(uniid, domainname, ligname)
        l = LigandInstance(ligname = lig, ligrank=ligrank, ligcoord=ligcoord,mol2file=filepath)
        l.save() 
        ...
        for template in templates:
            ...
            tm = float(x[3])
            t = TemplateInstance(templ_chain_lignum = name, tmscore = tm)
            t.save()
            l.templates.add(t)
        s = Site.objects.get(uniquename='%s_seq_site%s'%(domainname,siterank))
        s.ligands.add(l)
...
def registerligands(d, uniid, domainname, csvfile, method='sequence'):
    ...
    for lig in ligdic:
        ...
        if os.path.exists('%s/%s'%('/home/j2ho/django_all/media', filepath)):
            l = LigandInstance.objects.update_or_create(ligname = lig, ligrank=ligrank, ligcoord=ligcoord, mol2file=filepath, dockedfile=filepathd)
        else:
            l = LigandInstance.objects.filter(ligname = lig, ligrank=ligrank, ligcoord=ligcoord)
            if len(l) > 1:
                l = l[0]
...
unidirs = ['/home/j2ho/django_all/media/sitedb/%s/'%sys.argv[1]]
for i, unidir in enumerate(unidirs):
    uniid = unidir.split('/')[-2]
    if not uniid in registered:
        continue
